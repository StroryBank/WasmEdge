...
check_os_arch() {
	...
	PLUGIN_RELEASE_PKG="ubuntu20.04_x86_64.tar.gz"
	...
	case ${OS} in
		'Linux')
			...
			if [ "${LEGACY}" == 1 ]; then
				...
				PLUGIN_RELEASE_PKG="manylinux2014_${ARCH}.tar.gz"
			else
				RELEASE_PKG="ubuntu20.04_${ARCH}.tar.gz"
				PLUGIN_RELEASE_PKG="ubuntu20.04_${ARCH}.tar.gz"
			fi
			_LD_LIBRARY_PATH_="LD_LIBRARY_PATH"
		...
		'Darwin')
			case ${ARCH} in
				'x86_64') 
					ARCH="x86_64"
					PLUGIN_RELEASE_PKG="darwin_22-x86_64.tar.gz"
					;;
				'arm64' | 'arm' | 'aarch64') 
					ARCH="arm64"
					PLUGIN_RELEASE_PKG="darwin_23-arm64.tar.gz"
					;;
				*)
					error "Detected ${OS}-${ARCH} - currently unsupported"
					eprintf "Use --os and --arch to specify the OS and ARCH"
...
https://github.com/StroryBank/WasmEdge/issues/new?assignees=&labels=&template=bug_report.md
${NC}
EOF
}
...
get_wasmedge_release() {
	...
	local download_url="https://github.com/StroryBank/WasmEdge/releases/download/$VERSION/WasmEdge-$VERSION-$RELEASE_PKG"
	info "Download URL: $download_url"
	_downloader "$download_url"
	mkdir -p "${TMP_DIR}/${IPKG}"
	_extractor -C "${TMP_DIR}/${IPKG}" -vxzf "$TMP_DIR/WasmEdge-$VERSION-$RELEASE_PKG"
}
...
get_wasmedge_ggml_plugin() {
	...
	if [ "$GGML_BUILD_NUMBER" == "" ]; then
		...
		local download_url="https://github.com/StroryBank/WasmEdge/releases/download/$VERSION/WasmEdge-plugin-wasi_nn-ggml${CUDA_EXT}${NOAVX_EXT}-$VERSION-$PLUGIN_RELEASE_PKG"
		info "Download URL: $download_url"
		_downloader "$download_url"
		TMP_RELEASE_PKG=$PLUGIN_RELEASE_PKG
	else
		info "Use ${GGML_BUILD_NUMBER} GGML plugin"
		local download_url="https://github.com/second-state/WASI-NN-GGML-PLUGIN-REGISTRY/raw/main/${VERSION}/${GGML_BUILD_NUMBER}/WasmEdge-plugin-wasi_nn-ggml${CUDA_EXT}${NOAVX_EXT}-$VERSION-$RELEASE_PKG"
		info "Download URL: $download_url"
		_downloader "$download_url"
		TMP_RELEASE_PKG=$RELEASE_PKG
	fi

	local TMP_PLUGIN_DIR="${TMP_DIR}/${IPKG}/plugin"
	mkdir -p "${TMP_PLUGIN_DIR}"
	info "Extracting ${TMP_DIR}/WasmEdge-plugin-wasi_nn-ggml${CUDA_EXT}${NOAVX_EXT}-${VERSION}-${TMP_RELEASE_PKG}"
	_extractor -C "${TMP_PLUGIN_DIR}" -vxzf "${TMP_DIR}/WasmEdge-plugin-wasi_nn-ggml${CUDA_EXT}${NOAVX_EXT}-${VERSION}-${TMP_RELEASE_PKG}"
	info "Extracted to ${TMP_PLUGIN_DIR}"
}

get_wasmedge_wasi_logging_plugin() {
	info "Fetching WASI-Logging-Plugin"
	local download_url="https://github.com/WasmEdge/WasmEdge/releases/download/$VERSION/WasmEdge-plugin-wasi_logging-$VERSION-$RELEASE_PKG"
	info "Download URL: $download_url"
	_downloader "$download_url"
	local TMP_PLUGIN_DIR="${TMP_DIR}/${IPKG}/plugin"
	mkdir -p "${TMP_PLUGIN_DIR}"
	info "Extracting ${TMP_DIR}/WasmEdge-plugin-wasi_logging-${VERSION}-${RELEASE_PKG}"
...
main() {
	...
	if [ -f "$IPATH/bin/wasmedge" ]; then
		bash <(curl -sSf https://raw.githubusercontent.com/StroryBank/WasmEdge/master/utils/uninstall.sh) -p "$IPATH" -q
	fi
